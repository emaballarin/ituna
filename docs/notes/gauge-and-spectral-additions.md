...
1. **The gauge group is `O(L)`, and iTuna has no class for it.** The available classes are
   ...
   `Linear` measures _subspace_ agreement; what is needed is _geometry_ agreement.
...
2. **The operator `K` is never examined.** iTuna's API is sklearn-transformer-shaped — `fit(X)`,
   ...
   spectral consistency: two runs that converged to _different_ approximately-Koopman-invariant
   ...
   gauge" from "different subspaces" requires comparing gauge-_invariants_ of `K` directly.
...
| Pairwise fit is `estimator.fit(X=X[i], y=X[j])` per **ordered** pair — forward and backward maps are independent fits, _not_ mutual inverses                                 | `ituna/metrics.py:337`              |
...
| `ConsistencyEnsemble.fit` hardwires the embeddings into the consistency transform — the transform _only_ ever receives embeddings                                            | `ituna/estimator.py`                |
...
| The upstream parity harness hardcodes `CLASSES = ["Identity", "Permutation", "Linear", "Affine"]`                                         | `tools/upstream_parity/probe.py:26` | A new indeterminacy class is invisible to the parity check by construction. This is _why_ section 5's "out of scope" holds — not an assumption about additive changes in general                                     |
...
(Schönemann, _Psychometrika_ 31(1), 1966.) Restricting to the rotation subgroup `SO(L)` instead
...
### 3.4b [ADDED] What a low `Orthogonal` score does _not_ prove
...
not a geometry, which is either a real identifiability failure _or_ slack isotropy, and the two are
...
| 5   | **Strictly stronger than `Linear`** | **[CORRECTED]** `Y = X @ A` with **`A = diag(3, 1, 1, 1, 1)` specifically** → `Linear().fit(X, Y).score(X, Y) ≈ 1.0` while `Orthogonal().fit(X, Y).score(X, Y) < 0.99`. This test _is_ the justification for the class; do not soften the threshold — pin `A` instead |
...
_without_ alignment, so they answer the question embedding alignment cannot — did the runs find the
...
Chirality blindness follows from _similarity_ invariance — `S K S = S K S⁻¹` for `S = S⁻¹` — and
...
assignment for `(h', h)` is the _inverse_ permutation of the one for `(h, h')`, so the lower triangle
...
- **[CORRECTED] Chirality is invisible — and so is every other similarity.** Eigenvalues are
  invariants of the _similarity_ class of `K`, so any `K ↦ S K S⁻¹` contributes exactly zero to this
  ...
  out of scope. Note that this blindness is the _same property_ that makes the module work at all:
...
**[CORRECTED] on test 4.** The stated `≈ ε (rtol 1e-6)` holds on _raw_ distances — measured
...
`tools/upstream_parity/` needs no _reconciliation_, because `probe.py:26` enumerates the four
...
runs found different invariant subspaces and _no_ embedding alignment class will make the comparison
meaningful.
...
- Do **not** modify `PairwiseConsistency`, `ConsistencyEnsemble`, the backends, or `utils` _in
  service of these additions_. Section 1 establishes that neither addition requires it.
...
1. **Procrustes solved in the wrong orientation.** `Q = U Vᵀ` from `M = Xᵀ Y` aligns `X → Y`; the
   ...
   `R²`, and no error. Test 3.5#1 (exact recovery of a _known_ `Q0` on a non-symmetric `X`) catches
...
3. **`Orthogonal` reported as an improvement when it is only a different bias.** Both `Linear` and
   ...
   settles it out-of-sample. Note this is _distinct_ from the slack-isotropy confound in 3.4b, which
